    ...
    <div class="page">
        ...
        <div class="two-col" style="align-items:start;">
            ...
            <div class="card">
                ...
                <div style="font-size:12px;color:var(--ink-3);margin-bottom:16px;">Must-haves narrow the pool. Weights rank
                    the results.</div>
                        ...
                        <div class="form-group">
                            <div class="flex items-center justify-between">
                                ...
                                <span style="font-size:12px;font-weight:500;"
                                    id="lbl-{{ $key }}">{{ old('weights.' . $key, $w['default']) }}</span>
                            </div>
                            <input type="range" name="weights[{{ $key }}]" min="0" max="5" step="1"
                                value="{{ old('weights.' . $key, $w['default']) }}"
                                oninput="document.getElementById('lbl-{{ $key }}').textContent=this.value" style="width:100%;">
                        </div>
                    @endforeach
            ...
            <div>
                <div style="font-size:14px;font-weight:500;margin-bottom:14px;">
                    @if(isset($results)) {{ $results->count() }} match{{ $results->count() !== 1 ? 'es' : '' }} found
                    @else Your matches will appear here @endif
                </div>
                    ...
                    @else
                        @foreach($results as $i => $item)
                            @php
                                ...
                                $bgColor = ['dog' => '#E1F5EE', 'cat' => '#FBEAF0', 'rabbit' => '#E6F1FB', 'bird' => '#EAF3DE', 'hamster' => '#FFF3E0', 'other' => '#F5F4F0'][$pet->species] ?? '#F5F4F0';
                            @endphp
                            <a href="{{ route('pets.show', $pet) }}" style="display:block;margin-bottom:10px;">
                                <div class="card" style="{{ $i === 0 ? 'border:1.5px solid var(--green);' : '' }}padding:14px 16px;">
                                    <div class="flex items-center gap-12">
                                        ...
                                        <div
                                            style="width:52px;height:52px;border-radius:10px;overflow:hidden;flex-shrink:0;background:{{ $bgColor }};display:flex;align-items:center;justify-content:center;font-size:28px;">
                                            @if($imgUrl)
                                        ...
                                        <div style="flex:1;">
                                            ...
                                            <div
                                                style="margin-top:6px;height:4px;background:var(--border);border-radius:4px;overflow:hidden;">
                                                <div
