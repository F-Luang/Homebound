    ...
    <div class="page">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:24px;">
            <div class="card" style="text-align:center;padding:20px 16px;">
                <div style="font-size:32px;font-weight:600;color:var(--green);font-family:'Lora',serif;">{{ $totalPets }}
                </div>
                <div style="font-size:12px;color:#888;margin-top:4px;">Total pets</div>
            </div>
            <div class="card" style="text-align:center;padding:20px 16px;">
                <div style="font-size:32px;font-weight:600;color:#3C3489;font-family:'Lora',serif;">{{ $totalAdopted }}
                </div>
                <div style="font-size:12px;color:#888;margin-top:4px;">Adopted</div>
            </div>
            <div class="card" style="text-align:center;padding:20px 16px;">
                <div style="font-size:32px;font-weight:600;color:var(--green);font-family:'Lora',serif;">
                    {{ $totalAvailable }}
                </div>
                <div style="font-size:12px;color:#888;margin-top:4px;">Available</div>
            </div>
            <div class="card" style="text-align:center;padding:20px 16px;">
                <div style="font-size:32px;font-weight:600;color:var(--amber);font-family:'Lora',serif;">
                    {{ $totalApplications }}
                </div>
                        ...
                        @foreach($applicationsPerMonth as $month)
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;height:100%;">
                                <div style="flex:1;display:flex;align-items:flex-end;width:100%;">
                                    <div style="width:100%;height:{{ round(($month['total'] / $maxApp) * 100) }}%;background:var(--green-light);border-radius:4px 4px 0 0;border-bottom:2px solid var(--green);transition:height 0.4s;min-height:4px;"
                                        title="{{ $month['total'] }} applications"></div>
                                </div>
                                <div style="font-size:10px;color:#888;text-align:center;white-space:nowrap;">{{ $month['label'] }}
                                </div>
                ...
                @else
                    ...
                    <div style="display:flex;align-items:flex-end;gap:8px;height:120px;">
                        @foreach($adoptionsPerMonth as $month)
                            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;height:100%;">
                                <div style="flex:1;display:flex;align-items:flex-end;width:100%;">
                                    <div style="width:100%;height:{{ round(($month['total'] / $maxAdopt) * 100) }}%;background:#EEEDFE;border-radius:4px 4px 0 0;border-bottom:2px solid #3C3489;transition:height 0.4s;min-height:4px;"
                                        title="{{ $month['total'] }} adoptions"></div>
                                </div>
                                <div style="font-size:10px;color:#888;text-align:center;white-space:nowrap;">{{ $month['label'] }}
                                </div>
                                <div style="font-size:11px;font-weight:500;">{{ $month['total'] }}</div>
            ...
            <div class="card">
                ...
                @forelse($speciesBreakdown as $species)
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                        <div
                            style="width:28px;height:28px;border-radius:8px;background:{{ $color['bg'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            <img src="{{ asset('assets/icons/' . $color['icon']) }}" alt="{{ $species->species }}"
                        ...
                        <div style="flex:1;">
                            ...
                            <div style="height:6px;background:#f0f0ec;border-radius:4px;overflow:hidden;">
                                <div
                                    style="height:100%;width:{{ round(($species->total / $totalSpecies) * 100) }}%;background:{{ $color['border'] }};border-radius:4px;transition:width 0.5s;">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                ...
                @forelse($statusBreakdown as $item)
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                        <div style="width:10px;height:10px;border-radius:50%;background:{{ $color['color'] }};flex-shrink:0;">
                        </div>
                        <div style="flex:1;">
                            <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                                <span style="font-size:13px;">{{ ucfirst(str_replace('_', ' ', $item->status)) }}</span>
                                <span style="font-size:12px;color:#888;">{{ $item->total }}
                                    ({{ round(($item->total / $totalStatus) * 100) }}%)</span>
                            </div>
                            <div style="height:5px;background:#f0f0ec;border-radius:4px;overflow:hidden;">
                                <div
                                    style="height:100%;width:{{ round(($item->total / $totalStatus) * 100) }}%;background:{{ $color['color'] }};border-radius:4px;opacity:0.7;">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="two-col">
            ...
            <div class="card">
                ...
                @forelse($mostAppliedPets as $i => $pet)
                    <div
                        style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,0.06);">
                        <div
                            style="width:24px;height:24px;border-radius:50%;background:var(--green-light);color:var(--green-dark);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;flex-shrink:0;">
                            {{ $i + 1 }}
                        </div>
                        <div style="flex:1;">
                        ...
                        <div style="font-size:13px;font-weight:500;color:var(--green);">{{ $pet->applications_count }} <span
                                style="font-size:11px;color:#888;font-weight:400;">apps</span></div>
                    </div>
                @empty
                    <div class="text-muted">No applications yet.</div>
                @endforelse
            ...
            <div class="card">
                ...
                @forelse($recentAdoptions as $app)
                    <div
                        style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,0.06);">
                        <div
                            style="width:32px;height:32px;border-radius:50%;background:#EEEDFE;color:#3C3489;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:600;flex-shrink:0;">
                            {{ strtoupper(substr($app->user->name ?? 'U', 0, 1)) }}
                        </div>
                        <div style="flex:1;">
                            <div style="font-size:13px;font-weight:500;">{{ $app->user->name ?? 'Unknown' }}</div>
                            <div style="font-size:11px;color:#888;">adopted {{ $app->pet->name ?? 'Deleted pet' }}</div>
                        </div>
