@extends('layouts.app')
@section('title', 'Application — ' . $application->pet->name)

@section('content')
        <div class="page">
            <div style="margin-bottom:16px;">
                <a href="{{ route('applications.index') }}" class="text-muted" style="font-size:13px;">← Back to
                    applications</a>
            </div>
                        ...
                        <div class="flex items-center justify-between" style="margin-bottom:20px;">
                            <div>
                                ...
                                <div style="font-size:13px;color:#888;margin-top:2px;">
                                    Applied for <strong style="color:#1a1a18;">{{ $application->pet->name }}</strong>
    ...
    if ($application->status === 'rejected')
        $currentIndex = -1;
                        ...
                        @if($application->status === 'rejected')
                            <div
                                style="background:#FAECE7;border:0.5px solid #F0997B;border-radius:8px;padding:12px 14px;font-size:13px;color:#993C1D;">
                    ...
                    <div class="card" style="margin-bottom:16px;">
                        ...
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
                            <div
                                style="width:44px;height:44px;border-radius:50%;background:#E1F5EE;color:#0F6E56;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:600;flex-shrink:0;">
                                {{ strtoupper(substr($application->user->name, 0, 1)) }}
                            </div>
                            <div>
                                <div style="font-size:14px;font-weight:500;">{{ $application->user->name }}</div>
                        ...
                        @if($application->notes)
                            ...
                            <div
                                style="font-size:13px;color:#444;line-height:1.7;background:#F5F4F0;border-radius:8px;padding:12px 14px;">
                                {{ $application->notes }}
                            </div>
                        @else
                        ...
                        <div class="card" style="margin-bottom:16px;">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="badge badge-{{ $application->meetGreet->status === 'completed' ? 'approved' : 'review' }}">
                                ...
                                @if($application->meetGreet->volunteer)
                                    <div>
                                        ...
                                        <div style="font-size:13px;font-weight:500;">{{ $application->meetGreet->volunteer->name }}
                                        </div>
                                    </div>
                                @endif
                ...
                <div>
                    <div class="card">
                        ...
                        {{-- Pet photo --}}
                        @php $primary = $application->pet->images->firstWhere('is_primary', true) ?? $application->pet->images->first(); @endphp
                        @if($primary)
                        ...
                        @else
                            <div
                                style="height:140px;background:#E1F5EE;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:52px;margin-bottom:14px;">
