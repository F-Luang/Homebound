                                            ...
                                            <div class="card">
                                                ...
                                                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:8px;">
                                                    @foreach($pet->images as $img)
                                                        <div onclick="openLightbox('{{ str_starts_with($img->path, 'http') ? $img->path : str_starts_with($img->path, 'http') ? $img->path : asset('storage/' . $img->path)}', '{{ $pet->name }}')"
                                                            ...
                                                            <img src="{{ str_starts_with($img->path, 'http') ? $img->path : str_starts_with($img->path, 'http') ? $img->path : asset('storage/' . $img->path) }}" alt="{{ $pet->name }}"
                                                                style="width:100%;height:90px;object-fit:cover;display:block;">
                                                            @if($img->is_primary)
                                                                <div
                                                                    style="background:#1D9E75;color:white;font-size:10px;font-weight:500;text-align:center;padding:3px 0;">
                                                                    Main photo
                                                                </div>
                                                            @endif
                                                        </div>
